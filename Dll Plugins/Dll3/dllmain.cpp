...
class test_plugin3 : public plugin_base {
    void execute() override {
        std::cout << "this is first plugin of second dll" << std::endl;
    }
};

class test_plugin4 : public plugin_base {
    void execute() override {
        std::cout << "this is second plugin of second dll" << std::endl;
    }
};

class test_plugin_constructor : public plugin_constructor {
    plugin_base* operator()(void) override { return new test_plugin3(); };
    std::string get_class_name() override { return "plugin 1_A"; };
};

class test_plugin_constructor2 : public plugin_constructor {
    plugin_base* operator()(void) override { return new test_plugin4(); };
